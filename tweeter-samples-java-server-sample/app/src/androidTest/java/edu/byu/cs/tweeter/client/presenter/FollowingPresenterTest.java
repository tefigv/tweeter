...
import edu.byu.cs.tweeter.client.presentors.FollowingPresenter;
import edu.byu.cs.tweeter.client.presentors.PagedPresenter;
...
public class FollowingPresenterTest {
    ...
    public void setup() {
        ...
        fakeAuthToken = new AuthToken("abc-123-xyz-789", 3456765);
        ...
        // FollowService.
        FollowingPresenter followingPresenter = new FollowingPresenter(followingViewMock, fakeUser);
        followingPresenterSpy = Mockito.spy(followingPresenter);

        followingServiceMock = Mockito.mock(FollowService.class);
        Mockito.doReturn(followingServiceMock).when(followingPresenterSpy).startService();
    }
    ...
    public void testLoadMoreItems_CorrectParamsPassedToStatusService() {
        ...
        Answer<Void> manyFolloweesAnswer = new Answer<Void>() {
            ...
            public Void answer(InvocationOnMock invocation) throws Throwable {
                ...
                Assertions.assertEquals(limit, PagedPresenter.PAGE_SIZE);
                Assertions.assertEquals(lastFollowee, followingPresenterSpy.getLastItem());

                FollowService.GetFollowObserver observer = invocation.getArgument(4);
                observer.successfulGetFollow(followees ,true);
                return null;
            }
        };
    ...
    public void testLoadMoreItems_GetFolloweesSuccess() throws InterruptedException {
        ...
        Mockito.verify(followingViewMock).startingLoad();
        Mockito.verify(followingViewMock).endingLoad();
    ...
    public void testLoadMoreItems_GetFolloweesFailsWithErrorMessage() throws InterruptedException {
        ...
        Mockito.verify(followingViewMock).startingLoad();
        Mockito.verify(followingViewMock).endingLoad();
        Mockito.verify(followingViewMock).showErrorMessage("Failed to retrieve followees: " + "failure message");
    ...
    public void testLoadMoreItems_GetFolloweesFailsWithExceptionMessage() throws InterruptedException {
        ...
        Mockito.verify(followingViewMock).startingLoad();
        Mockito.verify(followingViewMock).endingLoad();
        Mockito.verify(followingViewMock).showErrorMessage("Failed to retrieve followees because of exception: " + "The exception message");
        Mockito.verify(followingViewMock, Mockito.times(0)).addItems(Mockito.any());
    }
