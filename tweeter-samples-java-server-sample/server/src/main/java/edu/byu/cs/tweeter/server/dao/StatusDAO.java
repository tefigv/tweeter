...
public class StatusDAO extends IStatusDAO {

    private static final String tableName = "status-table";

    DynamoDbTable<Status> table;
    private Key getKey(String alias, long timestamp_num) {
        table = enhancedClient.table(tableName, TableSchema.fromBean(Status.class));
        return Key.builder().partitionValue(alias).sortValue(timestamp_num).build();
    }

    public void putItem(String alias, long timestamp_num, String urls,String mentions, String status_text,
                        String timestamp_string,String user_name){
        ...
        if(status != null){
            ...
            Status newStatus = new Status(alias,timestamp_num,urls, mentions,status_text,timestamp_string,user_name);
